...
public class GameClass implements Game {
    ...
    @Override
    public MoveOutput move(int x, int y, String dir, int nrMoves) {
        if (!mapE.isValidPosition(x, y)){
            return MoveOutput.INVALID_POSITION;
        } else if (!isValidDirection(dir)) {
            return MoveOutput.INVALID_DIRECTION;
        } else {
            Player player = findPlayer(x, y);
            if (player == null) {
                return MoveOutput.NO_PLAYER;
            } else if (nrMoves == RedPlayerClass.MAX_MOVES && !(player instanceof RedPlayerClass)){
                return MoveOutput.INVALID_MOVE;
            } else if (!isMovementPossible(dir, x, y)){
                return MoveOutput.OFF_MAP;
            } else {
    ...
    private boolean isMovementPossible(String dir, int x, int y){
        boolean result = switch(Directions.valueOf(dir)){
            case NORTH -> isValidPosition(x, y-1);
            case SOUTH -> isValidPosition(x, y+1);
            case EAST -> isValidPosition(x+1, y);
            case WEST -> isValidPosition(x-1, y);
        };
        return result;
    }
